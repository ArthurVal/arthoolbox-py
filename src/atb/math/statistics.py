...
def update_sum_squares(new_data, old_sum_squares, new_mean, old_mean):
    """Compute the update of sum of squares of differences from the mean.
...
class OnlineStatistics(object):
    ...
    def __init__(self, condition_lock=None):
        """Construct the Stat object with the provided  mutex."""
        self.updated = Condition(condition_lock)
        self.reset()

    def __repr__(self):
        """Make a string repr of the Stat object."""
        return str(self.stats)

    @property
    def number_of_measurement(self):
        """Return the current number of measurments done."""
        return self.__n

    @property
    def measurement(self):
        """Return the last measurment stored X."""
        return self.__measurement

    @measurement.setter
    def measurement(self, new_measure):
        """Update with a new measurment."""
        with self.updated:
            self.__n += 1

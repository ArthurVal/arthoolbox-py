...
def test_update_mean(data):
    ...
    for i, single_data in enumerate(data, start=1):
        computed_mean = arthstats.update_mean(
            new_data=single_data, old_mean=computed_mean, num_data=i
        )

        log.debug("{} -> X:{} | Mean: {}".format(i, single_data, computed_mean))

        assert math.isclose(
            st.mean(data[:(i)]), computed_mean, rel_tol=0.005
        ), (
            "The recursive mean computed on step n = {} doesn't match the"
            " arithmetic mean computed with statistics.mean"
        ).format(i)
...
def test_update_variance(data):
    ...
    for i, single_data in enumerate(data[1:], start=2):
        i_mean = arthstats.update_mean(
            new_data=single_data, old_mean=i_1_mean, num_data=i
        )
        ...
        log.debug(
            ("{} -> X:{} | Mean: {} | Variance: {}").format(
                i, single_data, i_mean, computed_variance
            )
        )
...
def test_update_sum_squares(data):
    ...
    for i, single_data in enumerate(data[1:], start=2):
        ...
        computed_variance, computed_sample_variance = (
            (computed_sum_squares / i),
            (computed_sum_squares / (i - 1)),
        )
...
def test_OnlineStatistics(data):
    ...
    log.debug("Use the OnlineStatistics class on {} data".format(len(data)))
    for i, single_data in enumerate(data, start=1):
        ...
        assert math.isclose(
            st.mean(data[:(i)]), online_data.mean, rel_tol=0.005
        ), (
            "The OnlineStatistics mean computed on step n = {} doesn't match"
            " the arithmetic mean computed with statistics.mean"
        ).format(i)

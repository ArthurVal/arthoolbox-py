...
def test_sample_attr(function, log=module_log.getChild("attr")):
    ...
    for attr_name in [
        ...
        assert hasattr(function, attr_name), (
            "Sampled function should have {} attribute".format(attr_name)
        )
...
def test_sample_attr_init(function, log=module_log.getChild("attr_init")):
    ...
    for attr_name, expected_init_value in [
        ...
        log.debug(
            "{}: {} (expecting: {})".format(
                attr_name, getattr(function, attr_name), expected_init_value
            )
        )
        assert getattr(function, attr_name) == expected_init_value, (
            "Sampled function attr {} should be equals to {} initially".format(
                attr_name, expected_init_value
            )
        )
    log.info("{0:*^50}".format(" DONE "))
...
def test_sample_output(function, input_args, log=module_log.getChild("output")):
    ...
    for input_used in input_args:
        ...
        log.debug(
            "sampled f({}) = {} (expecting: {})".format(
                input_used, output_obtained, output_expected
            )
        )
...
def test_sample_computation(
    function, input_args, log=module_log.getChild("computation")
):
    ...
    log.debug(
        "period.number_of_measurement: {} (expecting: {})".format(
            function.period.number_of_measurement, (len(input_args) - 1)
        )
    )
    assert function.period.number_of_measurement == (len(input_args) - 1), (
        "Sampled function number_of_measurement {} should be equals to {}".format(
            function.period.number_of_measurement, len(input_args) - 1
        )
    )
    ...
    assert math.isclose(function.period.mean, sleep_time_sec, rel_tol=0.05), (

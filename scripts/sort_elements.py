...
"""Sort (stable) items from a range in ascending order.
...
def __is_using_stdin():
    """Indicate if stdin is active (i.e. using a pipe)."""
    return bool(select.select([sys.stdin], [], [], 0)[0])


def __all_stdin_elements():
    """Create a Generator over all lines from stdin until EOF."""
    try:
        while True:
            yield input()
    except EOFError:
...
def __when_output(predicate, handle):
    """Decorate f, calling handle(args) when predicate(f(args)) is TRUE."""
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default='(.*)',
        type=__regex_arg,
        help="Regex containing AT LEAST one matching group (default: '(.*)')."
        '\nThe matching group will be used as KEY when sorting elements.'
        '\nExample:'
        "\n-kf '^(.).*$': Use the FIRST character of each element."
        '\nWARNING: None matching element will be ignore.'
    )
    ...
    parser.add_argument(
        ...
        help='Expected key type (after filtering) used to perform the '
        'comparison when sorting (default: str).'
    )
    ...
    for elem in sorted(
            ...
            key=lambda m: convert_key(m.group(1))
    ):
        # Elem are VALID (i.e. != None) regex.matches object returned by
        # get_key_match:
        # .group(0) corresponds to the complete elem that matches the pattern
